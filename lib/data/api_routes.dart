class RemoteUrls {
  // static const String baseUrl = 'https://revx.ozrit.in/';
  static const String baseUrl = 'http://192.168.0.20:8080/';
  static const String userLogin = 'auth/login';
  static const String vendorRegister = 'auth/diagnostic-register';
  static const String vendorGetTests = 'diagnostic-admin/tests';
  static const String addTests = 'diagnostic-admin/tests';
  static const String vendorDeleteTests = 'diagnostic-admin/test-detail';
  static const String vendorUpdateTests = 'diagnostic-admin/test-detail';
  static const String vendorAddTests = 'diagnostic-admin/tests';
  static const String vendorGetCategories = 'diagnostic-admin/categories';
  static const String superAdminTests= 'diagnostic-admin/super-tests';
}